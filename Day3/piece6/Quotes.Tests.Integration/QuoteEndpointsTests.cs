...
/// so each test gets its own QuotesWebAppFactory → its own SqliteConnection
/// → its own in-memory database.  No shared state across tests.
/// </summary>
public sealed class QuoteEndpointsTests : IDisposable
{
    ...
    {
        _factory = new QuotesWebAppFactory();
