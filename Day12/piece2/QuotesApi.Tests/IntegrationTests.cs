...
/// <summary>
/// Verifies the Dapper author-report endpoint returns a correctly grouped response.
/// Uses the same SQLite in-memory factory — no Docker required.
/// </summary>
public sealed class AuthorReportTests : IClassFixture<QuotesApiFactory>
{
    private readonly HttpClient _client;

    public AuthorReportTests(QuotesApiFactory factory)
        => _client = factory.CreateClient();

    [Fact]
    public async Task DapperAuthorReport_Returns200_WithElapsedMsAndGroupedAuthors()
    {
        var resp = await _client.GetAsync("/api/author-report/dapper");
        ...
        // Response must carry a non-negative elapsedMs field.
        var elapsedMs = json.RootElement.GetProperty("elapsedMs").GetInt64();
        Assert.True(elapsedMs >= 0);

        // Seed data: 20 authors × 10 quotes each → window fn must produce 20 groups.
        var report = json.RootElement.GetProperty("report").EnumerateArray().ToList();
        Assert.Equal(20, report.Count);

        // Every group must carry quoteCount = 10 (COUNT computed at the DB tier).
        foreach (var group in report)
            Assert.Equal(10, group.GetProperty("quoteCount").GetInt32());
    }
}
