...
public sealed class QuoteEndpointsTests : IClassFixture<SqlServerFixture>, IDisposable
{
        ...
        int    userId = 1)
    {
        ...
        var q = await resp.Content.ReadFromJsonAsync<CreateQuoteResult>();
        return q!.Id;
    }
    ...
    public async Task GetQuotes_EmptyDb_Returns200WithEmptyList()
    {
        ...
        var quotes = await resp.Content.ReadFromJsonAsync<List<QuoteSummaryDto>>();
    ...
    public async Task GetQuoteById_ExistingId_Returns200WithQuote()
    {
        ...
        Assert.Equal(HttpStatusCode.OK, resp.StatusCode);
        var q = await resp.Content.ReadFromJsonAsync<QuoteSummaryDto>();
        Assert.NotNull(q);
