...
describe('QuotesListComponent', () => {
  ...
  it('renders a clickable item for each loaded quote', async () => {
    const fixture = TestBed.createComponent(QuotesListComponent);
    await flushInit(fixture);

    const items = fixture.nativeElement.querySelectorAll('.quote-item');
    expect(items.length).toBe(QUOTES.length);
  });
});
