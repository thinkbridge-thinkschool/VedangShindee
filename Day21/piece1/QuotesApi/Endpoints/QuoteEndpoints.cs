...
public static class QuoteEndpoints
{
    public static IEndpointRouteBuilder MapQuoteEndpoints(this IEndpointRouteBuilder app)
    {
            ...
            CancellationToken ct) =>
        {
            ...
            // A new quote invalidates paged list caches; individual quote caches are unaffected.
            ...
            CancellationToken ct) =>
        {
            ...
            // Evict both the individual-quote entry (tagged "quote:{id}") and all paged lists.
            ...
            // Enqueue the audit event; the BackgroundService writes it off the request thread.
            var ownerId = userId is not null && int.TryParse(userId, out var uid) ? (int?)uid : null;
            auditQueue.Enqueue(new QuoteAuditEvent("deleted", id, quote.Author, ownerId, DateTimeOffset.UtcNow));
