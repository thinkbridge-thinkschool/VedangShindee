...
public static class ServiceBusDemoEndpoints
{
    public static void MapServiceBusDemoEndpoints(this WebApplication app)
    {
        ...
        grp.MapPost("/publish", async (PublishRequest req, IQuoteEventPublisher publisher) =>
        {
        ...
        grp.MapPost("/publish-poison", async (PoisonRequest req, IQuoteEventPublisher publisher) =>
        {
            var evt = new QuoteCreatedEvent
            {
                QuoteId = -1,
        ...
        grp.MapPost("/publish-duplicate", async (DuplicateRequest req, IQuoteEventPublisher publisher) =>
        {
